...
switch ($event) {
	...
	if ($lasthostproblemid != 0) {
      ...
      mysql_select_db($sqldb) or die(mysql_error());
	    ...
	    $content = "$state: $servico. Ticket closed automatically through the event $eventzabbix.";
    ...
    case "DOWN":
		...
		case "PROBLEM":
			...
			if (!empty($session)) {
			    ...
			    $title = "$state: $servico! -  Event $eventzabbix automatically generated by Zabbix";
			    $content = "HOSTNAME: $eventhost. TRIGGER ID: $triggerid. TRIGGER STATUS: $state.";
			    if ($category != ''){
